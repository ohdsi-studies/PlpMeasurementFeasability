...
sql <- "WITH person_measurement AS (
  ...
  INNER JOIN @cdmSchema.concept AS c ON frequency.measurement_concept_id = c.concept_id
...
connection = connect(cdms[[1]]$connectionDetails)
...
optum_ehr2_bw <- optum_ehr2_bw %>% 
  mutate(UNIT_CONCEPT_NAME = ifelse(UNIT_CONCEPT_NAME == "No matching concept", 
                                    "no missing concept", 
                                    UNIT_CONCEPT_NAME))
...
optum_ehr2_bw$UNIT_LABEL <- as.factor(optum_ehr2_bw$UNIT_LABEL)
...
treemap_chart <- ggplot(optum_ehr2_bw, aes(area = PERCENTAGE, fill = UNIT_LABEL, 
  ...
  geom_treemap() +
  geom_treemap_text(colour = "white", place = "centre", size = 5) +
  theme_minimal() +
  ...
  theme(legend.position = "right", 
        ...
        plot.caption = element_text(hjust = 6, vjust=3))

print(treemap_chart)
ggsave("results/treemap_chart.png", treemap_chart, width = 10, height = 10)
